function [tide,xi,yi]=gom_tide_z(time_utc,tidal_model) 
% GOM_TIDE_Z function to plot the predicted tidal heights
% in the Gulf of Maine at a specified time.
%
% Usage: [tide,xi,yi]=gom_tide_vec([time_utc],[tidal_model]);
...
%                tide = predicted tide at the (xi,yi) points 
%
...
%
%                tidal_model = 'ADCIRC' (default) or 'QUODDY'
%
...
if (nargin<1),
  utc=datenum(now)+toffset/24;  % assume that NOW is in EDT  
   ...
   if ~isstr(time_utc) & length(time_utc)>=3,
      if length(time_utc)==3,
         utc=datenum(time_utc(1),time_utc(2),time_utc(3));
      else
         utc=datenum(time_utc(1),time_utc(2),time_utc(3),...
            time_utc(4),time_utc(5),time_utc(6));
      end
   else
...
case {'QUODDY'}
  ...
  [zamp,zpha,freq]=...
       quoddy_tide_interp_z(xi(igood),yi(igood),con_ids,bimonth);

case {'ADCIRC'}
  con_ids=[1:8]; % use Z0, O1, K1, N2, M2, S2, M4, M6 
  [zamp,zpha,freq]=...
        adcirc_tide_interp_z(xi(igood),yi(igood),con_ids);
otherwise
...
wi=tide_track_z(track,zamp,zpha,freq);
...
% The title
title(['Tidal Elevation from ' tidal_model],...
...
c2=caxis;
pclegend(c2,[.17 .70 .2 .03],gcf,'meters')
